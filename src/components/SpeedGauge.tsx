...
export default function SpeedGauge({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (active || displayRef.current > 0) {
        const needleAngle =
          startAngle + totalAngle * Math.min(displayRef.current / max, 1);
      ...
      const displayVal =
        displayRef.current >= 1
          ? displayRef.current.toFixed(displayRef.current >= 100 ? 0 : 1)
          : displayRef.current > 0
          ? (displayRef.current * 1000).toFixed(0)
          : "—";
      ...
      ctx.fillText(
        displayRef.current >= 1 || displayRef.current === 0
          ? displayVal
          : displayVal,
        cx,
        cy
      ...
      ctx.fillText(
        displayRef.current > 0 && displayRef.current < 1 ? "Kbps" : unit,
        cx,
        cy + size * 0.15
      );
  ...
  }, [value, max, unit, label, color, active, size]);
